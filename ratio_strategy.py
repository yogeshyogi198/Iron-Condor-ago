...
def _send_alert(state_before: dict, state_after: dict,
                signal: str, current_ratio: float, mode: str):
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if signal == "ENTRY":
        action = f"Sell {state_before['sell_symbol']} \u2192 Buy {state_before['buy_symbol']}"
        telegram_logger.send_telegram(
            f"\U0001f504 RATIO STRATEGY\n"
            f"\U0001f4ca Ratio: {current_ratio:.4f}\n"
            f"\U0001f4c8 Signal: {state_after['holding']}\n"
            f"\U0001f501 Action: {action}\n"
            f"\U0001f4b0 Quantity: {state_after['quantity']} units\n"
            f"\u23f0 Time: {now_str}\n"
            f"\U0001f4c4 Mode: {mode}",
            level="TRADE",
        )
    elif signal == "EXIT":
        held = state_before["holding"]
        entry_p = state_before["entry_price"]
        qty = state_before["quantity"]
        ltp = current_ratio  # placeholder, we pass actual LTP separately
        telegram_logger.send_telegram(
            f"\U0001f504 RATIO STRATEGY\n"
            f"\U0001f4ca Ratio: {current_ratio:.4f}\n"
            f"\U0001f4c8 Signal: EXIT {held}\n"
            f"\u23f0 Time: {now_str}\n"
            f"\U0001f4c4 Mode: {mode}",
            level="TRADE",
        )
...
def main():
    ...
    telegram_logger.send_telegram(
        f"\U0001f504 RATIO STRATEGY started ({mode_str} mode)\n"
        f"Holding: {state['holding'] or 'None'}",
        level="INFO",
    )
            ...
            if state["holding"]:
                ...
                if do_exit:
                    ...
                    telegram_logger.send_telegram(
                        f"\U0001f504 RATIO STRATEGY\n"
                        f"\U0001f4ca Ratio: {current_ratio:.4f}\n"
                        f"\U0001f4c8 Signal: EXIT {sell_symbol}\n"
                        f"\u2757 Reason: {exit_reason}\n"
                        f"\U0001f4b0 Qty: {qty} units | Entry: \u20b9{state['entry_price']:.2f} "
                        f"| Exit: \u20b9{ltp_sold:.2f}\n"
                        f"\U0001f4c8 P&L: \u20b9{pnl:+,.2f}\n"
                        f"\u23f0 Time: {now.strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"\U0001f4c4 Mode: {mode_str}",
                        level="TRADE",
                    )
            ...
            if not state["holding"]:
                ...
                if signal:
                    ...
                    if not paper_trade and qty > 0:
                        ...
                        if sell_qty > 0:
                            _place_cnc_order(kite, sell_symbol,
                                             sell_qty, "SELL")
                            time.sleep(2)
                    ...
                    telegram_logger.send_telegram(
                        f"\U0001f504 RATIO STRATEGY\n"
                        f"\U0001f4ca Ratio: {current_ratio:.4f}\n"
                        f"\U0001f4c8 Signal: {signal}\n"
                        f"\U0001f501 Action: {action_str}\n"
                        f"\U0001f4b0 Quantity: {qty} units @ \u20b9{buy_ltp:.2f}\n"
                        f"\u23f0 Time: {now.strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"\U0001f4c4 Mode: {mode_str}",
                        level="TRADE",
                    )
